...
function page(body, script = '') {
  return `<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>PTK Action Fixture</title></head>
<body>
...
if (new URLSearchParams(location.search).has('run')) eval('window.__fixtureExecuted = true');`);
  ...
  if (url.pathname === '/') {
    response.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
    response.end(page('<h1>PTK Action Fixture</h1><a href="/api/items?id=1">API item</a>'));
    return;
  }
...
server.listen(PORT, HOST, () => {
  process.stdout.write(`PTK Action fixture listening on http://${HOST}:${PORT}\n`);
});
